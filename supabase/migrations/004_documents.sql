...
CREATE TABLE documents (
  ...
  search_vector         TSVECTOR GENERATED ALWAYS AS (
    ...
    setweight(to_tsvector('arabic',  COALESCE(official_number, '')), 'B') ||
    setweight(to_tsvector('arabic',  COALESCE(summary_ar, '')), 'B')    ||
    setweight(to_tsvector('french',  COALESCE(summary_fr, '')), 'B')    ||
...
CREATE INDEX idx_documents_year             ON documents(EXTRACT(YEAR FROM document_date)::INTEGER);
