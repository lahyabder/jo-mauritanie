...
CREATE TABLE upload_jobs (
  ...
  processing_duration_ms INTEGER GENERATED ALWAYS AS (
    CASE
      WHEN processing_started_at IS NOT NULL AND processing_ended_at IS NOT NULL
      THEN EXTRACT(EPOCH FROM (processing_ended_at - processing_started_at))::INTEGER * 1000
      ELSE NULL
    END
  ) STORED,
